...
export default async function ProfilePage() {
  ...
  const [playerProfileRes, coachProfileRes, videosRes] = await Promise.all([
    isPlayer
      ? supabase
    ...
    isCoach
      ? supabase
          .from("coach_profiles")
          .select("is_verified, figc_card_url")
          .eq("id", user.id)
          .maybeSingle()
      : Promise.resolve({ data: null, error: null }),
    isPlayer
    ...
    <div className="min-h-[100dvh] bg-leva-bg pb-28">
      ...
      <section className="px-6 py-8 flex flex-col items-center text-center">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          {isCoach && !coachProfile?.is_verified && coachProfile?.figc_card_url && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-500/20 border border-amber-500/30 text-amber-300 text-xs font-medium rounded-full">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3">
        ...
        <section className="px-6 pb-6">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-white font-semibold text-sm">Richieste di certificazione</p>
              <p className="text-zinc-400 text-xs mt-0.5">
                Approva o rifiuta le richieste dei tuoi giocatori
              </p>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5 text-zinc-500 shrink-0">
      ...
      {isPlayer && !playerProfile?.is_verified && (
        <section className="px-6 pb-6">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-white font-semibold text-sm">Verifica il profilo</p>
              <p className="text-zinc-400 text-xs mt-0.5">
                Richiedi la certificazione al tuo allenatore FIGC
              </p>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5 text-zinc-500 shrink-0">
              <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
